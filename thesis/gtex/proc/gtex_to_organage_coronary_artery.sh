...
mkdir -p ./proc_data
for file in ../data/gene_tpm_2017-06-05_v8_artery_coronary.gct.gz; do
    organ="${file#../data/gene_tpm_2017-06-05_v8_}"
    organ="${organ%.gct.gz}"
    # cp $file ./proc_data/$organ.gct.gz
    # gzip -d ./proc_data/$organ.gct.gz
    # mv ./proc_data/$organ.gct ./proc_data/$organ.tsv
    ...
    cat ./proc_data/$organ.head.txt ./proc_data/$organ.HL.tsv.20 > ./proc_data/$organ.TEST.$1.tsv
    cat ./proc_data/$organ.head.txt ./proc_data/$organ.HL.tsv.80 > ./proc_data/$organ.TRAIN.$1.tsv
